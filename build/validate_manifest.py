...
CI gate. Fails non-zero with the file + field that broke. Stdlib-only
so the gate stays fast and cross-platform without an install step.

Schema is hand-written (no JSON Schema doc) for the same reason as
real-esrgan-serve's: small + stable + clearer error messages than
jsonschema's structured ones. If this grows past ~3 manifest types,
swapping to jsonschema becomes the right call.
"""
...
# Mirrors RunPod's REST enum, sampled 2026-04. null is allowed for
